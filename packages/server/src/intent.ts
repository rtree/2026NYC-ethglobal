...
const DEFAULT_START: StartConfig = { loopPeriodSec: 5, ttlMinutes: 10, watcherEnabled: true };
...
export async function setStartConfig(uid: string, intentId: string, cfg: Partial<StartConfig>) {
  ...
  const startConfig: StartConfig = {
    loopPeriodSec: clampInt(cfg.loopPeriodSec, 2, 60, doc.startConfig.loopPeriodSec),
    ttlMinutes: clampInt(cfg.ttlMinutes, 1, 60, doc.startConfig.ttlMinutes),

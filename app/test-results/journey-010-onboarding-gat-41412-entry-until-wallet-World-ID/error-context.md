...
- Name: journey.spec.ts >> 010 onboarding gate blocks entry until wallet + World ID
- Location: e2e/journey.spec.ts:12:1
...
Locator: getByText('human verified')
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for getByText('human verified')

```

```yaml
- banner:
  - link "I IntentOS Base mainnet":
    - /url: "#/"
  - text: running
  - link "0x0783…a6C9":
...
- main:
  ...
  - link "Result":
    - /url: "#/result"
  - text: 020 · Intent List
  - heading "Your Intents" [level=1]
  - paragraph: One active Intent per Owner. The active Intent is live on Base mainnet; review it on the dashboard or open the launch flow for a new one.
  - 'link "intent-abc running DCA USDC → WETH Cumulative spent 0.011 USDC · per-tx cap 0.002 USDC Executor #1 Watcher"':
    - /url: "#/dashboard"
    - text: intent-abc running
    - heading "DCA USDC → WETH" [level=3]
    - paragraph: Cumulative spent 0.011 USDC · per-tx cap 0.002 USDC
    - text: "Executor #1 Watcher"
  - link "new → Run a new Intent Speak an intent, generate the Agent Package, mint the Executor, delegate via EIP-7702, fund the gas vault, and start. IntentBuilder → mint → 7702 → start":
    - /url: "#/launch"
    - text: new →
    - heading "Run a new Intent" [level=3]
    - paragraph: Speak an intent, generate the Agent Package, mint the Executor, delegate via EIP-7702, fund the gas vault, and start.
    - text: IntentBuilder → mint → 7702 → start
  - paragraph: IntentOS · ETHGlobal NYC 2026
```

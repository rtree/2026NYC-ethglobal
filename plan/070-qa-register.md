...
| LLM-001 | IntentBuilder | Fixed | Vertex failures log + return `llm:"mock"` (UI labels "scripted"). `INTENTOS_LLM_STRICT=1` makes production hard-fail instead of falling back. | Demo reliability vs production truthfulness. | Default = graceful logged fallback; set `INTENTOS_LLM_STRICT=1` to hard-fail. | Manual review |
...
| API-005 | Reset intent context | Fixed | Reset carries the current `intentId` (Intent List loads the active Intent doc); off-chain intent is marked stopped. | Intent List now loads `/api/intents`. | Keep `intentId` on reset. | `app/e2e/data-wiring.spec.ts` |
| API-006 | Start conditions | Fixed | `POST /api/runtime/start` consumes StartConfig: marks the intent live + records a bounded schedule (startedAt / autoStopAt / plannedTicks, capped at 12). Periodic on-chain execution stays the manual guarded-trade path; a Cloud Scheduler tick is documented future work (must stay bounded). | Runtime launch orchestration now exists. | Keep ticks bounded (MAX_PLANNED_TICKS); a real periodic loop needs Cloud Scheduler/Tasks. | `app/e2e/data-wiring.spec.ts` (start-runtime test) |
| UI-001 | Active Intent card | Fixed | Intent List active card renders the live intent's `intentId` + `title` from `/api/intents`. | Intent List loads the active Intent doc. | Keep API fixture values different from demo constants. | `app/e2e/data-wiring.spec.ts` |
| UI-002 | Live Console title | Fixed | Live Console heading shows the live intent `intentId` + `title` from `/api/intents`. | Console title derives from the active Intent doc. | Keep title API-driven. | `app/e2e/data-wiring.spec.ts` |
| UI-003 | Owner address display | Fixed | Live Console Owner EOA uses `/api/state.delegate`. | Removed the `ADDR.owner` shortcut in the live view. | Render `state.delegate` for live owner/delegate. | `app/e2e/data-wiring.spec.ts` |
| UI-004 | Runtime/funding claims | Fixed | Funding/launch badges reflect actual vault + `delegated` state (no false "Owner-funded"/"live on Base"). Runtime substrate row labeled as substrate, not a running claim. | Badges now driven by `/api/state`. | Keep badges state-driven. | `app/e2e/data-wiring.spec.ts` |
| UI-005 | ENS/ERC-8004 registration | Fixed | Identity cards mark ENS/ERC-8004 as **planned** (not claimed-registered) until on-chain; AgentNFT + ExecutionContract shown as real. | No registry read path yet; labels are honest. | Add a registry/ENS read API to upgrade "planned" to verified. | Manual review |
| UI-006 | Token pair display | Fixed | Guard/summary token pair derives from `state.guard.tokenA/tokenB` via `tokenSymbol()` (falls back to USDC/WETH for the MVP pair). | Known-token map added in `config.ts`. | Extend the map when new pairs are supported. | Manual review |
| ROUTE-001 | Direct routes | Fixed | `#/intents`, `#/launch`, `#/console` enforce wallet + Firebase sign-in + World ID via an `App.tsx` route guard; direct links show Onboarding until the gate passes. | Route guard added. | Keep the guard in front of every protected destination. | `app/e2e/data-wiring.spec.ts` (deep-link test) |
| WORLDID-001 | Human proof | Open (intentional) | World ID stays a dev simulation (sessionStorage flag) by product decision; real IDKit widget is deferred. | Hackathon scope. | Wire `@worldcoin/idkit` when `VITE_WORLDID_APP_ID` is set; gate logic already supports it. | n/a |

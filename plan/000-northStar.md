...
OpenClaw は Agent の頭と作業面である。OpenClaw が「やりたい」と判断しても、それだけでは資金は動かない。OpenClaw ができるのは、Agent Package に従って観測し、考え、許可された IntentOS tool を呼ぶことだけである。

IntentBuilder は、Owner の Natural Intent から OpenClaw Runtime に投入する Agent Package を生成する。
OpenClaw はその Agent Package を読み、AgentLoop を回し、IntentOS の typed tools を呼ぶ。
資金移動の final authority は EIP-7702 ExecutionContract と Hard Guardrails に残る。
...
OpenClaw は bounded onchain interaction を orchestration する AgentLoop である。
ただし OpenClaw に onchain authority は渡さない。
OpenClaw は IntentOS typed tools を呼び、IntentOS adapter と ExecutionContract が final authority を持つ。

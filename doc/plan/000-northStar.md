...
> Watcher構想は一旦横に置き、最初の製品は **Executorのみ** で成立させる。
> Plan整理は [120-maintenance-index.md](120-maintenance-index.md)、最初のIssueは
> [130-issue-pivot-x402-funded-executor.md](130-issue-pivot-x402-funded-executor.md) を参照する。

IntentOSのハッカソンMVPは、AIAgentによる取引の課題を、EIP-7702によってOwnerのWalletでの資金保持・ガードレール設置を実現しつつ、取引の監視を別Agentが行うことで解決するレイヤープロトコルとして実証した。

メンテナンスフェーズでは、この実証で得たコード資産を使い、x402で支払いを受け、その支払いをAgent FundとしてTradingAgentの稼働資金にする。Intentが固まったらExecutor Agent NFTをSpawnし、Cloud Run上のExecutor Runtimeが、そのFundからgas代とTradingをまかなって稼働する。Agent NFTが他者に移転した場合、残りのFund / claim / runtime authorityも一緒に移る設計へ進める。

## 2026-06-15 メンテナンス North Star: x402-funded Executor Agent

次の挑戦の主語は、Executor-onlyのTradingAgentである。
...
1. Executor RuntimeはFundを自由に持たない。支出はAgentFund / Hard Guardrails / runtime bindingで必ず制限される。
2. x402で受け取ったFundは、gas、Trading、refund、transferの状態が追跡できる。
...
原則は **request は上位層から下位層へ流れるが、authority は Chain 層に留まる** である。上位層（LLM / adapter / SessionKey）が壊れても、下位層（ExecutionContract と Hard Guardrails）が資金を守る。
...
同じ「古さ」でも止める層が違う。expiry は Hard Guard が機械判定し、quote の鮮度（freshness）は Semantic Guard が意味判定する。
...
## 6. ハッカソンMVP スコープ（履歴: Executor + 単一 Watcher）

> メンテナンスメモ: この章はETHGlobal NYC 2026で通したMVPの履歴として残す。
> 2026-06-15以降の製品Critical Pathは、x402-funded Executor-only TradingAgentであり、Watcherはfuture moduleとして扱う。
> 新しい作業単位は [130-issue-pivot-x402-funded-executor.md](130-issue-pivot-x402-funded-executor.md) を起点にする。
...
> メンテナンスメモ: 以下はハッカソンMVPのEIP-7702 Owner-custody modelである。
> Pivot後は、x402で支払われたAgent FundをNFT transferと一緒に移す必要があるため、
> `AgentNFT` transfer、Runtime Binding、AgentFund contract / escrow / ledger の関係を再設計する。

...
> the first product path is **Executor-only**. See [120-maintenance-index.md](120-maintenance-index.md)
> for plan organization and [130-issue-pivot-x402-funded-executor.md](130-issue-pivot-x402-funded-executor.md)
> for the first Issue.
...
The next product subject is an Executor-only TradingAgent.
...
- Watcher Agent, Watcher Runtime, quorum, tighten / freeze are outside the first product path and stay
  as future research assets.

### Code Facts Written Back

- The codebase already has IntentBuilder, Agent Package FIX, Firestore store, Firebase Auth, World ID
  gate, OpenClaw Gateway, KMS SessionKey, Relayer, Uniswap quotes, EvidenceCommitted, and AgentNFT mint
  foundations.
- `ExecutionDelegate7702` proved strong Hard Guardrails while funds sit in the Owner EOA. But an
  x402-funded product that requires "NFT transfer moves Fund with it" needs a new AgentFund contract /
  escrow / ledger authority boundary; Owner EOA balances alone do not satisfy transfer semantics.
- `AgentNFT` currently moves identity and Runtime usage right, not Fund custody. The pivot must bind
  tokenId, Fund, Runtime Binding, and current owner.
- Cloud Run runtime must become a bounded durable driver through Scheduler, Tasks, Jobs, or a dedicated
  session service rather than depending on a loop inside one HTTP request.
- Browser wallets blocked arbitrary EIP-7702 delegation signing. The x402 paid flow should avoid making
  that limitation the first product dependency.

### New Invariants

1. The Executor Runtime never has free custody. Spending is constrained by AgentFund, Hard Guardrails,
   and runtime binding.
2. x402-funded value has trackable gas, trading, refund, and transfer state.
3. The NFT owner controls Agent identity, Runtime usage right, and the claim on remaining Fund.
4. After NFT transfer, the old Runtime structurally cannot spend the Fund.
5. Cloud Run execution is bounded by ticks, gas, trade count, and Vertex cost.
6. Watcher is a future module, not a requirement for the first product.
...
## 6. Hackathon MVP Scope (Historical: Executor + single Watcher)

> Maintenance note: this section records the MVP shipped for ETHGlobal NYC 2026. The product critical
> path after 2026-06-15 is the x402-funded Executor-only TradingAgent; Watcher is treated as a future
> module. New work starts from [130-issue-pivot-x402-funded-executor.md](130-issue-pivot-x402-funded-executor.md).
...
> Maintenance note: the text below describes the hackathon MVP's EIP-7702 Owner-custody model. After
> the pivot, the x402-paid Agent Fund must move with NFT transfer, so `AgentNFT` transfer, Runtime
> Binding, and an AgentFund contract / escrow / ledger must be redesigned together.
